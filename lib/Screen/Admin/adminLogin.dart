import 'dart:async';
import 'package:car_rental/Screen/Admin/adminDashboard.dart';
...
class _AdminLoginState extends State<AdminLogin> {
 ...
 _showSuccessSnackBar(context) {
    ...
    if (datauser.length == 0) {
      ...
      if (response.statusCode == 200) {
        Constant.sharedPref = await SharedPreferences.getInstance();
         setState(() {
          Constant.sharedPref?.setBool("adminIsLoggedIn", true);
          isLoading = false;
        });
        isLoading
            ? Navigator.of(context, rootNavigator: true).pop('dialog')
  ...
  getData() async {
    setState(() {
      adminIsLoggedIn = Constant.sharedPref?.getBool("adminIsLoggedIn");
    });
  }
